...
-   **Internet-Connected AI**: Agent 4 uses the Gemini API's built-in Google Search grounding to fetch up-to-date, relevant information and sources for topics that require it.
...
2.  **Agent 4 (The Internet Researcher)**: When a heading is flagged by Agent 1, this agent performs a targeted search using the Gemini API's integrated Google Search tool. This fetches relevant information and provides source links in a single, efficient step.
...
-   **AI API**: Google Gemini (`gemini-2.5-flash`) with Google Search grounding
